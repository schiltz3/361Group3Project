...
class CreateCourseTest(TestCase):
    ...
    def setUp(self):
        ...
        tas = []
        instructor = AccountUtil.getAccountByID(AccountUtil.createInstructorAccount("Instructor", "password"))
    ...
    def test_badName(self):
        course = self.form(name="!@#$%^%", instructor=self.course["instructor"], tas=self.course["ta"])
    ...
    def test_blankName(self):
        course = self.form(name="", instructor=self.course["instructor"], tas=self.course["ta"])
        resp = self.client.post(self.TEMPLATE, course["form"])
        self.assertEquals("Name must not be blank.", resp.context[self.WARNING], msg="Failed to detect bad course name.")

    def test_badDescription(self):
        course = self.form(description="!@#$%^&*", instructor=self.course["instructor"], tas=self.course["ta"])
        resp = self.client.post(self.TEMPLATE, course["form"])
    ...
    def test_blankDescription(self):
        course = self.form(description="", instructor=self.course["instructor"], tas=self.course["ta"])
        resp = self.client.post(self.TEMPLATE, course["form"])
        self.assertEquals("Description must not be blank.", resp.context[self.WARNING], msg="Failed to detect bad course name.")

    def test_instructorNotFound(self):
        course = self.form(instructor="Mario")
        resp = self.client.post(self.TEMPLATE, course["form"])
        self.assertEquals("Instructor could not be found.", resp.context[self.ERROR], msg="Failed to detect nonexistent instructor.")

    def test_instructorIsNone(self):
        course = self.form()
        resp = self.client.post(self.TEMPLATE, course["form"])
        self.assertEquals("Instructor must not be blank.", resp.context[self.WARNING], msg="Failed to detect None instructor.")

    def test_TANotFound(self):
        ta = "turtle"
        course = self.form(instructor=self.course["instructor"], tas=[ta])
        resp = self.client.post(self.TEMPLATE, course["form"])
        self.assertEquals("TA '" + ta + "' does not exist.", resp.context[self.ERROR], msg="Failed to detect nonexistent TA.")
    
    def test_courseSuccess(self):
        course = self.form(instructor=self.course["instructor"], tas=self.course["ta"])
        resp = self.client.post(self.TEMPLATE, course["form"])
        self.assertEquals("Course created.", resp.context[self.MESSAGE], msg="Failed to detect successful course creation.")
