...
class CreateCourse(View):
    ...
    def post(self, request)
        ...
        if request.user.is_anonymous or request.user.account.authority == 0:
            return render(request, 'course/create.html', {'message': 'User is not authorized to create the course.',
        ...
        if not name:
            return render(request, 'course/create.html',
                          {'message': 'Course name must not be blank.', 'instructors': AccountUtil.getInstructors()})
