...
class DeleteCourseTest(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(self.amount):
            course = CourseUtil.getCourseByID(CourseUtil.createCourse(name="Course" + str(i), description="Description"))
    ...
    def test_deleteCourse(self):
        ...
        self.assertIsNone(CourseUtil.getCourseByName(courseName), msg="Failed to delete a course.")
    ...
    def test_deleteMany(self):
        ...
        if limit > 0:
            ...
            for i in range(limit):
                self.assertIsNone(CourseUtil.getCourseByName(self.courseNames[i]), msg="Failed to delete a selection of courses.")

    def test_deleteAll(self):
        self.client.post(self.TEMPLATE, {"courses" : self.courseNames})
        self.assertIsNone(CourseUtil.getAllCourses(), msg="Failed to delete all courses")
    ...
    def test_keepNonDeleted(self):
        ...
        for name in self.courseNames:
            if name != courseName:
                self.assertIsNotNone(CourseUtil.getCourseByName(name), msg="Non-deleted course is missing from database.")

    def test_failToDelete(self):
        course = "Yay2021"
        resp = self.client.post(self.TEMPLATE, {"courses": [course]})
        self.assertEquals("Failed to delete: " + course, resp.context[self.ERROR], msg="Failed to detect a fake course deletion.")

    def test_emptySelection(self):
        resp = self.client.post(self.TEMPLATE, {"courses" : []})
        self.assertEquals("No courses were selected.", resp.context[self.WARNING], msg="Failed to warn about no courses being selected.")
