...
class NewAccountTest(TestCase):
    ...
    def test_admin(self):
        response = self.client.post("account/create.html", {"username": "new", "password": "password",
                                                            "authority": "1", "message": "test"})
        self.assertEqual("Administrator account 'new' created", response.context.get("message"),
                         "admin confirmation message not given")

    def test_instructor(self):
        response = self.client.post("account/create.html", {"username": "new", "password": "password",
                                                            "authority": "2"})
        self.assertEqual("Instructor account 'new' created", response.context.get("message"),
                         "instructor confirmation message not given")

    def test_ta(self):
        response = self.client.post("account/create.html", {"username": "new", "password": "password",
                                                            "authority": "3"})
        self.assertEqual("TA account 'new' created", response.context.get("message"),
    ...
    def test_noUsername(self):
        ...
        self.assertEqual(1, (AccountUtil.getAllAccounts()).count(), "new user created even though no username entered")

    def test_noPassword(self):
        self.client.post("account/create.html", {"username": "new", "password": "", "authority": "1"})
        self.assertEqual(1, (AccountUtil.getAllAccounts()).count(), "new user created even though no password entered")

    def test_usernameTaken(self):
        response = self.client.post("account/create.html", {"username": "user", "password": "password",
                                                            "authority": "1"})
        self.assertEqual("username 'user' is already in use", response.context.get("message"),
                         "username already taken")

    def test_noNewDuplicate(self):
        self.client.post("account/create.html", {"username": "user", "password": "password", "authority": "1"})
