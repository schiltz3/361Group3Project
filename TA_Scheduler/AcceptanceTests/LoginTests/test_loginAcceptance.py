...
class LoginTests(TestCase):
    def setUp(self):
        ...
        AccountUtil.createInstructorAccount(
            username="instructor", password="pass123word"
        )
    ...
    def test_RedirectInstructor(self):
        resp = self.client.post(
            "/", {"username": "instructor", "password": "pass123word"}, follow=True
        )
        self.assertRedirects(resp, "/dashboard/instructor/")

    def test_RedirectTA(self):
        resp = self.client.post(
            "/", {"username": "ta", "password": "password"}, follow=True
        )
        self.assertRedirects(resp, "/dashboard/ta/")

    def test_InvalidUser(self):
        resp = self.client.post(
            "/", {"username": "someone", "password": "abc123"}, follow=True
        )
        self.assertEqual(
            "Invalid username", resp.context["error"], "Login failed, user not found"
        )

    def test_badPassword(self):
        resp = self.client.post(
            "/", {"username": "admin", "password": "abc123"}, follow=True
        )
        self.assertEqual(
            "Invalid password",
            resp.context["error"],
    ...
    def test_blankPassword(self):
        ...
        self.assertEqual(
            "Invalid password", resp.context["error"], "Must input password"
        )
